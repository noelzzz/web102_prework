...
function addGamesToPage(games) {
    ...
    games.forEach(element => {
        ...
        const display = `
            ...
            <h3 class="game-name"> ${element.name} </h3>
            
            <div class="game-description"> ${element.description} </div>
            <br>
            <div class="backers"> Backers: ${element.backers} </div>
        `
...
function sortAlphabetically(){
   ...
   let listSortedAlphabetically = GAMES_JSON.sort(function (game1, game2) {
        if (game1.name < game2.name) {
          return -1;
        }
        if (game2.name > game2.name) {
          return 1;
        }
        return 0;
    });
    ...
    addGamesToPage(listSortedAlphabetically);
}
...
let displayStr = unfundedGames.length == 1 ? 
`A total of ${totalRaised} has been raised for ${GAMES_JSON.length} games. 
Currently, ${unfundedGames.length} game remains unfunded. We need your help to fund these amazing games!`
: 
`A TOTAL of $${totalRaised.toLocaleString('en-US')} has been raised for ${GAMES_JSON.length} games. 
Currently, ${unfundedGames.length} games remain unfunded. We need your help to fund these amazing games!`;



// create a new DOM element containing the template string and append it to the description container
const descString = document.createElement("p");
descString.innerHTML = `${displayStr}`
...
topPledge.innerHTML = `${one.name}`;
firstGameContainer.appendChild(topPledge);

// do the same for the runner up item
const runnerUpPledge = document.createElement("div");
runnerUpPledge.innerHTML= `${two.name}`
